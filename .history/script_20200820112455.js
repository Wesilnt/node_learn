...
function newSubjectData(source, config) {
  global.etrialInput = source;

  let clinicalData = {
    eCollect_StudyID: config.eCollectStudyID,
    StudyOID: config.studyOID,
  };
  global.edcOutput = [{ ClinicalData: clinicalData }];

  let subject = { SubjectKey: source.subjects.screenNumber };
  subject["InvestigatorRef"] = {
    UserOID: systemContext[_.toUpper("tm-header-AccountName")],
  };
  subject.SiteRef = {
    eCollect_LocationID: config.siteRef["eCollectLocationID"],
  };

  clinicalData.SubjectData = [subject];
  global.edcSubject = subject;

  let subjectInit = null;
  let vs1 = source.subjects.visits.filter((v) => v.oid === "VS1");
  if (vs1.length > 0) {
    vs1 = vs1[0];

    let fco = vs1.forms.filter((f) => f.oid === "F_CO");
    if (fco.length > 0) {
      fco = fco[0];
      let igco = fco.groups.filter((g) => g.groupOid === "IG_CO");
      if (igco.length > 0) {
        igco = igco[0];
        if (igco.groupValues && igco.groupValues.length > 0) {
          let item = igco.groupValues[0].items.filter(
            (i) => i.itemName === "SUBJIN"
          );
          if (item.length > 0) {
            subjectInit = item[0].rawValue;
          }
        }
      }
    }
    ...
    ItemGroupData: [
      {
        ...
        ItemData: [
          {
            ItemOID: "SUBJID",
            Value: source.subjects.screenNumber,
          },
          {
            ItemOID: "SUBJINIT",
            Value: subjectInit,
  ...
  subject["eCollect_SubjectFormData"] = subjectForm;
  global.systemContext[_.toUpper("tm-header-tenantId")] = config.extra.tenantId;
  global.systemContext[_.toUpper("tm-header-Source")] = config.extra.source;
