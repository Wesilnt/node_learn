...
module.exports =  function doMapping(input, ...mappings) {
    ...
    function doOneMapping(source, studyEventData, mapping, count) {
      // 强制转换 2020-08-20 对接eTrail
           if (count == 0) {
             let srcEvent = source.subjects.visits.filter(
               (v) => v.oid === "V1D1"
             )[0];
             ...
             srcEvent = source.subjects.visits.filter(
               (v) => v.oid === "V2D1"
             )[0];
               ...
               for (var _j = 0; _j < srcForms.length; _j++) {
                 if (srcForms[_j].oid === "F_VS1") {
                   ...
                   if (gs.length > 0) {
                     let _fgs = gs[0] || null;
                     if (_fgs) {
                       let _it = _fgs.items.filter(
                         (i) => i.itemName === "VSTIM"
                       );
                       let _v1 = _it[0]["rawValue"] ? _it[0]["rawValue"] : "";
                       if (_v1) {
                         _v1 = moment(parseInt(_v1)).format(
                           "YYYY-MM-DD HH:mm:ss"
                         );
                         _eachDestTree(
                           studyEventData,
                     ...
                     for (var _s = 0; _s < _gs.length; _s++) {
                       let _it = gs[_s].items.filter(
                         (i) => i.itemName === "VSTEMP"
                       );
                       if (_it.length > 0) {
                         let _val = _it[0]["rawValue"]
                           ? _it[0]["rawValue"]
                           : "";
                         if (_val) {
                           _eachDestTree(
